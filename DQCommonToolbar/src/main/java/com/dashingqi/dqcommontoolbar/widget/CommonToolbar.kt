...
class CommonToolbar : ConstraintLayout {
    ...
    init {
        var rootView = View.inflate(context, R.layout.common_toolbar_layout, this)
    ...
    private fun initAttribute(attributeSet: AttributeSet) {
        var obtainAttributeSet = context.obtainStyledAttributes(attributeSet, R.styleable.common_tool_bar)

        //设置标题
        obtainAttributeSet.getString(R.styleable.common_tool_bar_base_title)?.let { setTitleText(it) }
        obtainAttributeSet.getFloat(R.styleable.common_tool_bar_base_title_size, 16f)?.let { setTitleTextSize(it) }
        obtainAttributeSet.getColor(R.styleable.common_tool_bar_base_title_color, Color.BLACK)?.let { setTitleTextColor(it) }


        //设置左边布局
        if (obtainAttributeSet.hasValue(R.styleable.common_tool_bar_base_left_text)) {
            obtainAttributeSet.getString(R.styleable.common_tool_bar_base_left_text)?.let { setLeftText(it) }
        } else {
            setLeftText(getDefaultLeftText()!!)
        }

        if (obtainAttributeSet.hasValue(R.styleable.common_tool_bar_base_left_icon)) {
            obtainAttributeSet.getDrawable(R.styleable.common_tool_bar_base_left_icon)?.let { setLeftIcon(it) }
        } else {
            setLeftIcon(getDefaultLeftIcon()!!)
        }

        //设置右边布局
        if (obtainAttributeSet.hasValue(R.styleable.common_tool_bar_base_right_text)) {
            obtainAttributeSet.getString(R.styleable.common_tool_bar_base_right_text)?.let { setRightText(it) }
        } else {
            setRightText(getDefaultRightText()!!)
        }

        if (obtainAttributeSet.hasValue(R.styleable.common_tool_bar_base_right_icon)) {
            obtainAttributeSet.getDrawable(R.styleable.common_tool_bar_base_right_icon)?.let { setRightIcon(it) }
        } else {
